...
from tool_development_kit.slider import Slider
...
class IconListModel(QAbstractListModel):
    ...
    def setIconSize(self, size):
        ...
        self.dataChanged.emit(
            self.index(0, 0), self.index(len(self.__data), 0), [Qt.DecorationRole]
        )
...
class IconListView(QListView):
    ...
    def copySelectedIconName(self):
        ...
        hou.ui.setStatusMessage(
            f"Copied '{icon_name}' to clipboard", hou.severityType.ImportantMessage
        )
    ...
    def copySelectedIconFileName(self):
        ...
        hou.ui.setStatusMessage(
            f"Copied '{icon_file_name}' to clipboard", hou.severityType.ImportantMessage
        )
    ...
    def copySelectedIcon(self):
        ...
        if image:
            ...
            hou.ui.setStatusMessage(
                f"Copied '{image}' to clipboard", hou.severityType.ImportantMessage
            )
    ...
    def saveSelectedIcon(self):
        ...
        path, _ = QFileDialog.getSaveFileName(
            self, "Save image", name, filter="PNG (*.png)", initialFilter=name
        )
        ...
        if path and image:
            image.save(path)
            hou.ui.setStatusMessage(
                f"Saved '{image}' at '{path}'", hou.severityType.ImportantMessage
            )
...
class IconListDialog(QDialog):
    ...
    def getIconName(cls, parent=hou.qt.mainWindow(), title="Icons", name=None):
        ...
        if window.exec_() and window.icon_list_view.currentIndex().isValid():
            return (
                window.icon_list_view.currentIndex()
                .data(Qt.UserRole)
                .replace(".svg", "")
            )
