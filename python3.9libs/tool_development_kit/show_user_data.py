...
def prettify(text):
    ...
    if ini_wo_semicolon_regex.match(text):
        return "\n".join(
            r.expand(r"\1 = \2") for r in ini_wo_semicolon_regex.finditer(text)
        )
...
class UserDataWindow(QWidget):
    ...
    def setWordWrapEnabled(self, enable):
        if enable:
            self.user_data_view.setWordWrapMode(
    ...
    def _readData(self):
        ...
        if self.user_data_view.toPlainText() != data:
            ...
            if reselect:
                ...
                cursor.movePosition(
                    QTextCursor.Right, QTextCursor.MoveAnchor, selection_start
                )
                selection_length = abs(selection_start - selection_end)
                if selection_start > selection_end:
                    cursor.movePosition(
                        QTextCursor.Left, QTextCursor.KeepAnchor, selection_length
                    )
                else:
                    cursor.movePosition(
                        QTextCursor.Right, QTextCursor.KeepAnchor, selection_length
                    )
    ...
    def eventFilter(self, watched, event):
        if watched == self.user_data_view and event.type() == QEvent.Wheel:
            ...
            self._current_key == "nodeshape"
            and watched == self.user_data_view.viewport()
        ):
