...
def incrementHDAVersion(node, component, use_original_file):
    ...
    new_definition = hou.hda.definitionsInFile(new_file_path)[
        0
...
class NewVersionDialog(QDialog):
    ...
    def _updateDestFields(self):
        new_type_name = nextVersionTypeName(
            self.node.type().name(), self.comp_slider.value()
        )
    ...
    def _increment(self):
        incrementHDAVersion(
            self.node,
            self.comp_slider.value(),
            self.use_original_file_toggle.isChecked(),
        )
